...
function normalizeRows(csvText, source) {
  return parseCSV(csvText).map((row, idx) => {
    ...
    return {
      ...
      awards: row.Calificación?.includes('No recomendada') ? '' : row.Calificación || '',
      estrenoDate,
      weeksSinceRelease,
...
function computeViewModel() {
  ...
  const recommendedMovieKeys = new Set(recommendedSessions.map((s) => s.movieKey))
  const baseSessions = state.catalogMode === 'recommended' ? recommendedSessions : state.allSessions

  const movieStats = new Map()
  baseSessions.forEach((s) => movieStats.set(s.movieKey, (movieStats.get(s.movieKey) || 0) + 1))
  ...
  let movieCards = [...movieMap.values()].filter(moviePassesReleaseFilter)
  if (state.favoritesOnly) movieCards = movieCards.filter((m) => state.favorites.has(m.movieKey))
  if (state.mainView === 'catalog' && state.catalogAltOnly) movieCards = movieCards.filter((m) => !recommendedMovieKeys.has(m.movieKey))
  ...
  const modalMovie = state.modalMovieKey ? movieCards.find((m) => m.movieKey === state.modalMovieKey) || [...movieMap.values()].find((m) => m.movieKey === state.modalMovieKey) : null
...
function seatHint(session) {
  return session.roomCapacity ? `${session.cinema} • Sala ${session.room} • ${session.roomCapacity} seats` : `${session.cinema} • Sala ${session.room}`
}

function renderMovieBody(movie, asModal = false, recommendedMovieKeys = new Set()) {
  const grouped = groupSessionsByDay(movie.sessions)
  const recommendedBadge = recommendedMovieKeys.has(movie.movieKey) ? 'Recommended by trim logic' : 'Outside trimmed recommendations'

  const infoGrid = `
    <div class="info-grid">
      <div><label>Director</label><p>${movie.director || 'Unknown'}</p></div>
      <div><label>Release</label><p>${weekLabel(movie.weeksSinceRelease)}</p></div>
      <div><label>Language</label><p>${movie.originalLanguage || 'Unspecified'}</p></div>
      <div><label>Subtitles</label><p>${movie.subtitles || 'Not specified'}</p></div>
      <div><label>Duration</label><p>${movie.duration || 'n/a'} min</p></div>
      <div><label>Status</label><p>${recommendedBadge}</p></div>
    </div>
  `

  return `
    <article class="card panel ${state.favorites.has(movie.movieKey) ? 'favorite' : ''} ${asModal ? 'modal-card' : ''}" id="${movie.anchorId}">
      <div class="poster-wrap"><img src="${movie.posterUrl}" alt="${movie.movieTitle}" /></div>
      <div class="content">
        <div class="card-top">
          <h3>${movie.movieTitle}</h3>
          <button class="favorite-btn ${state.favorites.has(movie.movieKey) ? 'is-favorite' : ''}" data-favorite="${movie.movieKey}" title="Toggle favorite">★</button>
        </div>
        <p class="meta">${movie.director} • ${movie.year || 'Year n/a'} • ${movie.duration || 'n/a'} min • ${movie.sessions.length} sessions</p>
        <p class="release-pill">${weekLabel(movie.weeksSinceRelease)}</p>
        ${asModal ? infoGrid : ''}
        <p>${movie.synopsis}</p>
        <div class="tags">
          <span class="${cinemaTag(movie.cinema)}">${movie.cinema}</span>
          <span>${movie.originalLanguage || 'Language n/a'}</span>
          ${movie.subtitles ? `<span>${movie.subtitles}</span>` : ''}
          ${movie.rating ? `<span>${movie.rating}</span>` : ''}
        </div>
        <div class="links">
          ${movie.trailerUrl ? `<a href="${movie.trailerUrl}" target="_blank" rel="noreferrer">Trailer</a>` : ''}
          ${movie.filmUrl ? `<a href="${movie.filmUrl}" target="_blank" rel="noreferrer">Details</a>` : ''}
        </div>
        <details class="sessions-expand" ${asModal ? 'open' : ''}>
          <summary>Upcoming sessions (${movie.sessions.length})</summary>
          ${Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([date, list]) => `
            <div class="session-day-group">
              <h5>${date}</h5>
              <ul>${list.map((s) => `<li title="${seatHint(s)}"><strong>${s.time}</strong><span class="${cinemaTag(s.cinema)}">${s.cinema}</span><span>Sala ${s.room || 'n/a'}</span></li>`).join('')}</ul>
            </div>
          `).join('')}
        </details>
      </div>
...
function renderCatalog(movieCards, recommendedMovieKeys) {
  if (!movieCards.length) {
    return '<div class="empty panel"><h3>No movies match the current filters.</h3><p>Try widening date, release, language, or favorites filters.</p></div>'
  }
  return `<section><h2>Film Catalog</h2><div class="cards">${movieCards.map((m) => renderMovieBody(m, false, recommendedMovieKeys)).join('')}</div></section>`
}
          ...
          <article class="timeline-day panel">
            ...
            ${Object.entries(hourMap).sort(([a], [b]) => a.localeCompare(b)).map(([hour, list]) => `
              <details class="hour-block" open>
                ...
                <ul>
                  ${list.map((s) => `
                    <li class="timeline-item ${state.favorites.has(s.movieKey) ? 'favorite' : ''}" title="${seatHint(s)}">
...
function renderModal(modalMovie, recommendedMovieKeys) {
  ...
  return `
    <div class="modal-backdrop" id="modalBackdrop">
      <div class="modal-shell panel" role="dialog" aria-modal="true">
        ...
        <div class="modal-content">${renderMovieBody(modalMovie, true, recommendedMovieKeys)}</div>
      </div>
    </div>
  `
}
...
function render() {
  ...
  const { movieCards, scheduleByDayHour, modalMovie, favoritesCount, recommendedMovieKeys } = computeViewModel()
  ...
  root.innerHTML = `
    ...
    <div class="app-shell ${state.sidebarHidden ? 'sidebar-hidden' : ''}">
      <aside class="sidebar panel">
        ...
        <div class="sidebar-content">
          ...
          <div class="row toggles-row">
            <label class="check"><input id="favoritesOnly" type="checkbox" ${state.favoritesOnly ? 'checked' : ''}/> Favorites only</label>
            <label class="check ${state.mainView === 'catalog' ? '' : 'disabled'}"><input id="catalogAltOnly" type="checkbox" ${state.catalogAltOnly ? 'checked' : ''} ${state.mainView === 'catalog' ? '' : 'disabled'} /> Catalog-only: non-recommended</label>
          </div>
          ...
          <div class="row favorite-tools">
            <label>Favorites in view (${favoritesCount})</label>
      ...
      <main class="main-content">
        ...
        ${state.mainView === 'catalog' ? renderCatalog(movieCards, recommendedMovieKeys) : renderSchedule(scheduleByDayHour)}
      </main>
    </div>
    ${renderModal(modalMovie, recommendedMovieKeys)}
  `
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-open-modal]').forEach((btn) => {
    btn.onclick = () => {
      state.modalMovieKey = btn.dataset.openModal
      render()
    }
  })
  ...
  document.getElementById('favoritesOnly').onchange = (e) => { state.favoritesOnly = e.target.checked; render() }
  document.getElementById('catalogAltOnly').onchange = (e) => { state.catalogAltOnly = e.target.checked; render() }
