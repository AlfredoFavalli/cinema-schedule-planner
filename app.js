...
function computeWeeksSinceRelease(estrenoDate) {
  ...
  const diffDays = Math.floor((nowUTC - relUTC) / (1000 * 60 * 60 * 24))
...
function computeViewModel() {
  ...
  const filteredSessions = workingSessions.filter((s) => (
    ...
    && state.bucketFilter.has(s.bucket)
  ...
  const favoritesCount = movieCards.filter((m) => state.favorites.has(m.movieKey)).length
  return { movieCards, scheduleByDayHour, scheduleSessions, favoritesCount }
}

function chipList(items, set, key, labelTransform = (v) => v) {
  return items
    .map((item) => `<button class="chip ${set.has(item) ? 'active' : ''}" data-type="${key}" data-value="${item}">${labelTransform(item)}</button>`)
    .join('')
}
...
function renderCatalog(movieCards) {
  ...
  return `<section><h2>Film Catalog</h2><div class="cards">${movieCards.map((m) => {
    const grouped = groupSessionsByDay(m.sessions)
    return `
      <article class="card panel ${state.favorites.has(m.movieKey) ? 'favorite' : ''}" id="${m.anchorId}">
                  ...
                  ${list.map((s) => `
                    <li class="timeline-item ${state.favorites.has(s.movieKey) ? 'favorite' : ''}">
                      <div class="timeline-main">
                        <button class="jump-link" data-jump="${s.anchorId}" data-movie-key="${s.movieKey}">${s.movieTitle}</button>
...
function render() {
  ...
  const { movieCards, scheduleByDayHour, favoritesCount } = computeViewModel()
    ...
    <div class="app-shell ${state.sidebarHidden ? 'sidebar-hidden' : ''}">
      <aside class="sidebar panel">
        <div class="sidebar-head">
          ...
          <button id="toggleSidebar" class="ghost">${state.sidebarHidden ? 'Show' : 'Hide'}</button>
        </div>
        ...
        <div class="sidebar-scroll">
          <div class="row two-col">
            <div>
              ...
              <select id="mainView"><option value="catalog">Film Catalog</option><option value="schedule">Schedule Timeline</option></select>
            </div>
            <div>
          ...
          <div class="row favorite-tools">
            ...
            <div class="tool-actions">
              <button id="exportFavorites">Export JSON</button>
              <label class="upload">Import JSON<input id="importFavorites" type="file" accept="application/json" /></label>
            </div>
          </div>
        </div>
      </aside>
  ...
  document.getElementById('mainView').value = state.mainView
...
function jumpToMovie(anchorId) {
  state.mainView = 'catalog'
  render()
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-favorite]').forEach((btn) => {
    btn.onclick = () => {
      ...
      if (state.favorites.has(movieKey)) state.favorites.delete(movieKey)
      else state.favorites.add(movieKey)
      saveFavorites()
      render()
    }
  })
  ...
  document.querySelectorAll('[data-jump]').forEach((btn) => {
    btn.onclick = () => jumpToMovie(btn.dataset.jump)
  })
  ...
  document.getElementById('mainView').onchange = (e) => { state.mainView = e.target.value; render() }
